...
//        self.configuration["route"] = "chnroutes"
    ...
    @objc func save() {
        ...
        if let result = ConfigurationValidator.validate(configuration: self.configuration) {
            let alertController = UIAlertController.init(title: "Error", message: result, preferredStyle: .alert)
            
            alertController.addAction(UIAlertAction.init(title: "OK", style: .cancel, handler: { (action) -> Void in
        ...
        self.providerManager?.saveToPreferences(completionHandler:
                                                    { (error) -> Void in
    ...
    {
        if let nonNil = configuration[property], !(nonNil is NSNull)
        {
            textField.text = String(describing: nonNil)
        }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath)
    {
        ...
        if (indexPath.section == 0)
        {
            if (indexPath.row == 9)
            {
                if let controller = SimpleTableViewController.init(labels: ["Default", "CHNRoutes"], values: ["default", "chnroutes"], initialValue: self.configuration["route"] as? NSObject, selectionBlock: { (result) -> Void in
            ...
            let alertController = UIAlertController.init(title: nil, message: "Delete this configuration?", preferredStyle: .alert)
            alertController.addAction(UIAlertAction.init(title: "Delete", style: .destructive, handler: { (action) -> Void in
                self.providerManager?.removeFromPreferences(completionHandler: { (error) -> Void in
                    self.navigationController?.popViewController(animated: true)
                })
            }))
            alertController.addAction(UIAlertAction.init(title: "Cancel", style: .cancel, handler: { (action) -> Void in
            }))
            self.present(alertController, animated: true)
        }
    }
